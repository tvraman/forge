...
(defun forge--db-maybe-update (db version)
  (emacsql-with-transaction db
    ...
    (when (= version 3)
      ...
      (pcase-dolist (`(,table . ,schema) forge--db-table-schemata)
        (when (memq table '(notification
          ...
          (emacsql db [:create-table $i1 $S2] table schema)))
      (emacsql db [:alter-table issue   :add-column marks :default $i1] eieio-unbound)
      (emacsql db [:alter-table pullreq :add-column marks :default $i1] eieio-unbound)
    ...
    (when (= version 6) ; FIXME
      ...
      (emacsql db [:alter-table repository :add-column milestones :default $i1]
               eieio-unbound)
